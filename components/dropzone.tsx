...
export default function Dropzone() {
  ...
  const ignoreKeys = useRef<string[]>([])
  ...
  const convert = async (): Promise<any> => {
    ignoreKeys.current = []
    let tmp_actions = actions.map(elt => ({...elt,is_pending:true}));
    setActions(tmp_actions);
    setIsConverting(true);
    for (let action of tmp_actions) {
      if (ignoreKeys.current.includes(action.file_name)) continue
      try { 
        tmp_actions = tmp_actions.map((elt) => {
          if (elt.file_name === action.file_name) {
            return {
              ...elt,
              is_converting: true,
              is_pending: false
        ...
        tmp_actions = tmp_actions.filter((elt) => !ignoreKeys.current.includes(elt.file_name))
        setActions(tmp_actions);
        setProgress(0)
      } catch (err) {
        tmp_actions = tmp_actions.map((elt) =>
  ...
  const deleteAction = (action: Action): void => {
    if (is_converting)
      ignoreKeys.current = ignoreKeys.current.concat(
        action.file_name
      )
  ...
  const load = async () => {
    const ffmpeg_response: FFmpeg = await loadFfmpeg(setProgress);
          ...
          >
            ...
            {action.is_error ? (
              ...
              <div className="text-muted-foreground text-md flex items-center gap-4">
                ...
                <Selectformat action={action} updateAction={updateAction} />
              </div>
            )}
